...
class BaseModel:
    ...
    def __init__(self, ticker, num_of_days, train_valid_test_split_rate=0.8, train_valid_split_rate=0.5):
        self.ticker = ticker
        self.data = data_utils.get_data(ticker=ticker)
        self.normalized_data = data_utils.normalize_data(self.data)
        ...
        self.weights_file = self.ticker + '.weights.best.hdf5'
    ...
    def fit(self, epochs):
        model_checkpointer = ModelCheckpoint(filepath='weights/' + self.weights_file,
                                             verbose=1,
                                             save_best_only=True)
